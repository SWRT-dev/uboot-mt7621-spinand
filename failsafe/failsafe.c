...
int write_firmware_failsafe(size_t data_addr, uint32_t data_size)
{
	...
	return ubi_upgrade_image((const void *)data_addr, data_size);
#endif
}
	...
	if (status == HTTP_CB_NEW) {
		...
		if (buff) {
			...
			if (size_ptr) {
				...
				n = snprintf(size_str, sizeof(size_str), "%ld",
					fw->size);
...
{
	int ret;
#if defined(CONFIG_UBI_BOOT_SUPPORT)
	extern int fw_upgrader_init(void);
